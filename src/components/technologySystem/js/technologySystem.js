...
export function getTechnologySystem() {
  const url = debug ? '/api/getTechnologySystem' : 'http://luogjytest.top/api/getTechnologySystem';
...
export function getArticleList(currentPage, cid) {
  const url = debug ? '/api/getArticleListByTechnologySystem' : 'http://luogjytest.top/api/getArticleListByTechnologySystem';
  return axios.get(url, {
